...
test('session persists after browser restart', async ({browser, loggedInPage}) => {
    ...
    await new NavbarComponent(newPage).expectLoggedInUI(); // should we keep it as is?
});

test('invalid password should give error', async ({page}) => {
...
test('expired token logs the user out', async ({page, loggedInPage}) => {
...
test('logout clears the session', async ({page, loggedInPage}) => {
    await new NavbarComponent(page).logout();
    const token = await loggedInPage.getStoredToken();
    expect(token).toBeNull();
});

test('loggedIn user visiting login page redirects to homepage', async ({page, loggedInPage}) => {
